...
function writeField(
  ...
  switch (type) {
    case 'i32':
      return writer.writeInteger(n)
    case 'u32':
      return writer.writeCardinal(n >>> 0)
    case 'i16':
      return writer.writeSmallInt(n)
    case 'u16':
      return writer.writeWord(n)
    case 'i8':
      return writer.writeByte(n)
    case 'f64':
      return writer.writeDouble(n)
  }
}
...
export function writeTObject12(
  ...
  for (const [name, type] of TOBJECT_V12_HEADER) {
    writeField(
      writer,
      type,
      (object as unknown as Record<string, number | boolean>)[name],
    )
  }
