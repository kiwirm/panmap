// Deep-inspect single symbol across all round-trip paths.
import fs from 'node:fs/promises'
import os from 'node:os'
import path from 'node:path'
import { fileURLToPath } from 'node:url'
import { read as readMap, write as writeMap, ocad } from '../src/index.ts'

const HERE = path.dirname(fileURLToPath(import.meta.url))
const FIX = process.argv[2] ?? 'hillmorton-6275ebc'
const NUM = Number(process.argv[3] ?? '501010')
const dir = path.join(HERE, 'parity-fixtures', FIX)
const OCD = path.join(dir, 'mapper.ocd')

async function main() {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'ds-'))
  const src = await readMap(OCD)
  const xmapOut = path.join(tmp, 't.xmap')
  await writeMap(src, xmapOut)
  const rt = await readMap(xmapOut)
  const ocdOut = path.join(tmp, 't.ocd')
  await writeMap(rt, ocdOut)

  const ours = (await ocad.readRaw(ocdOut, { quietWarnings: true })).symbols
  const mapper = (await ocad.readRaw(OCD, { quietWarnings: true })).symbols
  const o = ours.find(s => s.symNum === NUM)
  const m = mapper.find(s => s.symNum === NUM)

  const strip = (v) => {
    if (Array.isArray(v)) return v.map(strip)
    if (v && typeof v === 'object') {
      const o = {}
      for (const [k, vv] of Object.entries(v)) {
        if (['_byteRange', 'warnings', 'iconBits', 'descriptionWords', 'symbolTreeGroup', 'mystery64', '_tail', '_fontNameBytes', 'size', 'filePos'].includes(k)) continue
        o[k] = strip(vv)
      }
      return o
    }
    return v
  }
  console.log('=== OURS ===')
  console.log(JSON.stringify(strip(o), null, 2))
  console.log('\n=== MAPPER ===')
  console.log(JSON.stringify(strip(m), null, 2))
  fs.rm(tmp, { recursive: true, force: true }).catch(() => {})
}
main()
